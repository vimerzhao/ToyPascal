...
public class CaseStatementParser extends StatementParser {
    ...
    public ICodeNode parse(Token token) throws Exception {
        ...
        // Loop to parse each CASE branch until the END
        // token or the end of the source file.
        while (!(token instanceof EofToken) && (token.getType() != END)) {
            ...
            // Look ofr the semicolon between CASE branches.
     ...
     * @param expressionType the CASE expression.
    ...
    private ICodeNode parseBranch(Token token,TypeSpec expressionType, HashSet<Object> constantSet) throws Exception {
        // Create an SELECT_BRANCH node and a SELECT_CONSTANT node.
        // The SELECT_BRANCH node adopts the SELECT_CONSTANT node as its first child.
     ...
     * @param token the current token.
     * @param expressionType the CASE expression.
     * @param constantsNode constantsNode the parent SELECT_CONSTANTS node.
     * @param constantSet the set of CASE branch constants.
     * @throws Exception if an error occurred.
     */
    private void parseConstantList(Token token, TypeSpec expressionType, ICodeNode constantsNode,
     ...
     * @param token the current token.
     * @param expressionType the CASE expression.
     * @param constantSet the set of CASE branch constant.
     ...
     * @param token (value) the current token value string.
